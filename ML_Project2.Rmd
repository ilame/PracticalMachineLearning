...
fit <- train(classe ~ ., data=ptrain1, method="rf", trControl=fitControl,allowParallel=TRUE)
```
...
### 
preds <- predict(fit, newdata=ptrain1)
inSampleError <- sum(preds != ptrain1$classe) * 100 / nrow(ptrain1)
```
...
```{r}
outOfSampleError <- sum(preds != ptrain2$classe) * 100 / nrow(ptrain2)
```
...
```{r, echo=TRUE}
varImpPlot(fit$finalModel, sort=TRUE, n.var=20, main="Importance for top 20 attributes", col="dark blue", pch=19)
...
```{r, echo=TRUE}
plot(fit$finalModel, log="y", main="Error rates vs number of trees")
```
...
fitControl <- trainControl(method="cv", number=3, verboseIter=F)
fit <- train(classe ~ ., data=ptrain, method="rf", trControl=fitControl,allowParallel=TRUE)
```
